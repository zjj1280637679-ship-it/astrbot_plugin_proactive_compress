...
- 调整：`astrbot_version` 由 `>=4.27,<5` 改为 `>=4.27`，只保留下限，不设大版本上限。

## 0.3.1 (2026-08-18)

- 新增：管理员触发压缩时可附带**本次压缩的额外提示词**（一次性，与全局
  `emphasis_instruction` 叠加，只对本次生效）：
  - `/compress 重点保留装修方案` —— 命令尾随文本作为额外要求；
  - 自然语言"压缩一下上下文，重点保留装修方案" —— 匹配意图短语后的内容
    自动提取为额外要求。
  - 实现：`_build_instruction(umo, extra)` 追加 `<extra_request>` 块；
    `_do_compress(..., extra=...)` 贯穿后台命令与自然语言两条路径。
- 调整：自然语言触发消息长度上限 40 → 80 字（允许意图短语 + 短附加要求）。
- 修复：默认自然语言正则中 `整理` 规则补充 `记录` 备选
  （`整理.*(上下文|对话|聊天)` → `整理.*(上下文|对话|聊天|记录)`），
  避免"整理一下聊天记录，多保留…"把"记录"误留在附加要求里。
